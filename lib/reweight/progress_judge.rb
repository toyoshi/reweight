...
module Reweight
  class ProgressJudge
    SCREENING_PROMPT = <<~PROMPT
      あなたはニュース記事の一次フィルタです。
      以下の記事タイトル一覧から、明らかに不採用な記事を除外し、残りを候補として選んでください。
      迷ったら候補に入れてください。判断が難しいものは次のステップで詳しく評価します。
      ...
      ## それ以外はすべて候補に入れる
      科学、動物、文化、スポーツ、経済、地域の話題など、上記に該当しないものはすべて候補にしてください。

      候補の記事番号をJSON配列で返してください。
      例: {"candidates": [1, 2, 5, 8, 12, ...]}
    PROMPT
